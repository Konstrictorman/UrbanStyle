...
class SalesDataProcessor:
    ...
    def get_categories(self):
        """
        Get list of unique product categories from the raw data.
        ...
        if self.raw_data is None:
            raise ValueError("Data not loaded. Call load_and_preprocess() first.")
        return sorted(self.raw_data['Product Category'].unique().tolist())
    
    def get_category_data(self, category):
        """
        Get weekly time series data for a specific category.
        
        This method filters the raw data by category and creates weekly aggregations
        with feature engineering specific to that category.
        
        Args:
            category (str): Product category name
            
        Returns:
            pd.DataFrame: Weekly time series data for the specified category
        """
        if self.raw_data is None:
            raise ValueError("Data not loaded. Call load_and_preprocess() first.")
        
        # Filter data for specific category
        category_data = self.raw_data[self.raw_data['Product Category'] == category].copy()
        
        if len(category_data) == 0:
            raise ValueError(f"No data found for category: {category}")
        
        # Create weekly aggregations for this category
        weekly_agg = category_data.groupby(pd.Grouper(key='Date', freq='W-SUN')).agg({
            'Quantity': 'sum',
            'Total Amount': 'sum',
            'Transaction ID': 'count',
            'Age': 'mean',
            'Gender': lambda x: (x == 'Female').sum() / len(x) if len(x) > 0 else 0
        }).reset_index()
        ...
        # Add lag features (previous weeks) - 4 weeks of lags
        lag_features = ['Weekly_Quantity', 'Weekly_Revenue', 'Transaction_Count']
        for feature in lag_features:
            for lag in range(1, 5):  # 4 weeks of lags
                weekly_agg[f'{feature}_lag_{lag}'] = weekly_agg[feature].shift(lag)
        ...
        print(f"Created weekly time series for {category}: {len(weekly_agg)} weeks")
        return weekly_agg
    
    def create_sequences_from_data(self, weekly_data, sequence_length=8, target_col='Weekly_Quantity', test_size=0.2):
        """
        Create sequences from specific weekly data with 80/20 train/test split.
        
        Args:
            weekly_data (pd.DataFrame): Weekly time series data
            sequence_length (int): Number of weeks to use as input sequence
        ...
        Returns:
            tuple: (X_train, X_test, y_train, y_test, train_data, test_data, feature_cols)
        """
        # Remove rows with NaN values (due to lags and rolling averages)
        clean_data = weekly_data.dropna().reset_index(drop=True)
        
        if len(clean_data) < sequence_length + 1:
            raise ValueError(f"Not enough data points: {len(clean_data)} < {sequence_length + 1}")
        
        # Calculate 80/20 split
        total_weeks = len(clean_data)
        train_weeks = int(total_weeks * (1 - test_size))
        test_weeks = total_weeks - train_weeks
        
        train_data = clean_data.iloc[:train_weeks].copy()
        test_data = clean_data.iloc[train_weeks:].copy()
        
        print(f"Category data split: {train_weeks} weeks for training ({train_weeks/total_weeks*100:.1f}%), {test_weeks} weeks for testing ({test_weeks/total_weeks*100:.1f}%)")
        
        # Select feature columns
        feature_cols = [col for col in clean_data.columns if col not in ['Week_Start', target_col]]
        ...
        # Check for constant features (where max == min) and handle them
        feature_range = feature_max - feature_min
        constant_features = feature_range < 1e-8
        
        # For constant features, set range to 1 to avoid division issues
        if np.any(constant_features):
            print(f"Warning: Found {np.sum(constant_features)} constant feature(s). Setting range to 1.0 for normalization.")
            feature_range[constant_features] = 1.0
        
        # Check for constant target
        target_range = target_max - target_min
        if target_range < 1e-8:
            print(f"Warning: Target values are constant (all {target_min}). Setting range to 1.0 for normalization.")
            target_range = 1.0
        
        # Normalize training data
        train_features_norm = (train_features - feature_min) / (feature_range + 1e-8)
        train_targets_norm = (train_targets - target_min) / (target_range + 1e-8)
        
        # Normalize test data using training statistics
        test_features = test_data[feature_cols].values.astype(np.float64)
        test_targets = test_data[target_col].values.astype(np.float64)
        test_features_norm = (test_features - feature_min) / (feature_range + 1e-8)
        test_targets_norm = (test_targets - target_min) / (target_range + 1e-8)
        
        # Create sequences from training data
        X_train, y_train = [], []
        for i in range(sequence_length, len(train_features_norm)):
            X_train.append(train_features_norm[i-sequence_length:i])
            y_train.append(train_targets_norm[i])
        
        # Create sequences from test data
        X_test, y_test = [], []
        for i in range(sequence_length, len(test_features_norm)):
            X_test.append(test_features_norm[i-sequence_length:i])
            y_test.append(test_targets_norm[i])
        
        # If test data is too small to create sequences, use validation split from training data
        if len(X_test) == 0:
            print(f"Warning: Test data too small ({len(test_data)} weeks) to create sequences (need {sequence_length + 1} weeks). Using validation split from training data.")
            ...
            # Update test_data to correspond to the validation sequences
            # The last val_size sequences predict the last val_size weeks
            # So test_data should be the last val_size weeks of the original train_data
            # (we need to account for sequence_length offset: sequences predict week i+sequence_length)
            # If we have sequences for weeks [sequence_length, sequence_length+1, ..., len-1],
            # and we take the last val_size sequences, those predict the last val_size weeks
            val_weeks_start = len(train_data) - val_size
            if val_weeks_start < sequence_length:
                val_weeks_start = sequence_length
            test_data = train_data.iloc[val_weeks_start:].copy()
            train_data = train_data.iloc[:val_weeks_start].copy()
            print(f"Adjusted split: {len(X_train)} training sequences, {len(X_test)} validation sequences")
        ...
        # Store normalization parameters in instance for denormalization
        self.feature_min = feature_min
        self.feature_max = feature_max
        self.target_min = target_min
        self.target_max = target_max
        
        return X_train, X_test, y_train, y_test, train_data, test_data, feature_cols
