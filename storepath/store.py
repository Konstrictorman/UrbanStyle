"""
Store Pathfinding Simulation with Q-Learning and Customer Movement Analysis

This module implements a comprehensive store simulation system that combines Q-learning
pathfinding algorithms with customer behavior modeling and analytics. The system provides
a visual interface for understanding optimal paths in retail environments and analyzing
customer movement patterns.

Key Features:
- Q-learning based pathfinding for multiple destinations
- Parallel training of Q-tables for different goals
- Customer simulation with realistic shopping behavior
- Heat map visualization of customer movement patterns
...
- Customer behavior modeling with shopping plans
- Real-time visualization and analytics
- Modular UI components for interaction
...
class Customer:
    """
    Customer class representing individual customers in the store simulation.
    
    This class models customer behavior including shopping plans, movement patterns,
    and analytics tracking. Each customer has a unique ID and follows a realistic
    shopping journey through the store.
    
    Attributes:
        id (int): Unique identifier for the customer
        current_position (tuple): Current (row, col) position in the store
        target_departments (list): List of departments the customer wants to visit
        visited_departments (list): List of departments already visited
        current_path (list): Current path being followed
        path_index (int): Index in the current path
        entrance (tuple): Entrance used by the customer
        exit (tuple): Exit used by the customer
        state (str): Current state ('entering', 'shopping', 'exiting', 'finished')
        shopping_time (float): Time spent shopping (minutes)
        total_time (float): Total time in store (minutes)
        departments_to_visit (int): Number of departments planned to visit
    """
    
    def __init__(self, customer_id):
        """
        Initialize a new customer with a unique ID.
        
        Args:
            customer_id (int): Unique identifier for this customer
        """
        self.id = customer_id
    ...
    def generate_shopping_plan(self):
        ...
        This method creates a realistic shopping plan by:
        - Selecting 1-3 departments to visit
        - Choosing a random entrance
        - Selecting the closest exit based on final department
        - Setting up the shopping journey
        
        Returns:
            None: Shopping plan is stored in instance attributes
        """
    ...
    def get_next_target(self):
        """
        Get the next department or exit the customer should visit.
        
        Returns:
            tuple: Next target position (row, col) or exit position
        """
    ...
    def reached_target(self):
        """
        Check if the customer has reached their current target.
        
        Returns:
            bool: True if customer has reached their current target, False otherwise
        """
    ...
    def move_to_next_position(self):
        """
        Move the customer to the next position in their current path.
        
        Returns:
            bool: True if movement was successful, False if at end of path
        """
...
def get_starting_location():
    """
    Get a random starting location for Q-learning episodes.
    
    This function generates a random starting position that is not a terminal state
    (goal or obstacle). It ensures the agent starts from a valid walkable position.
    
    Returns:
        tuple: Random starting position (row_index, col_index)
    """
...
def get_next_action(current_row_index, current_col_index, epsilon, q_values):
    ...
    This function implements the epsilon-greedy action selection strategy:
    - With probability epsilon: choose the best action (exploitation)
    - With probability (1-epsilon): choose a random action (exploration)
    
    Args:
        current_row_index (int): Current row position
        current_col_index (int): Current column position
        epsilon (float): Exploration rate (0.0 to 1.0)
        q_values (np.array): Q-table for the current goal
        
    Returns:
        int: Action index (0=up, 1=right, 2=down, 3=left)
    """
...
def get_next_location(current_row_index, current_col_index, action_index):
    """
    Calculate the next location based on the current position and action.
    
    This function applies the action to the current position and returns the new
    position, respecting the grid boundaries.
    
    Args:
        current_row_index (int): Current row position
        current_col_index (int): Current column position
        action_index (int): Action to take (0=up, 1=right, 2=down, 3=left)
        
    Returns:
        tuple: New position (new_row_index, new_col_index)
    """
...
def train_single_goal(target_goal, goal_idx, total_goals):
    """
    Train a single Q-table for a specific goal using Q-learning algorithm.
    
    This function implements the core Q-learning algorithm for a specific goal.
    It runs multiple episodes of training, updating the Q-table based on rewards
    and using epsilon-greedy exploration. The training is thread-safe and can
    run in parallel with other goals.
    
    Args:
        target_goal (tuple): Target goal position (row, col)
        goal_idx (int): Index of this goal in the goals dictionary
        total_goals (int): Total number of goals being trained
        
    Returns:
        None: Q-table is updated in-place in the global q_tables dictionary
    """
...
def on_train_button_click(button):
    """Callback for train button click"""
    global training
    if not training:
...
def on_set_start_click(button):
    """Callback for set start button click"""
    global pathfinding_mode, current_path
    pathfinding_mode = "start"
    current_path = []  # Clear current path

def on_clear_path_click(button):
    """Callback for clear path button click"""
    global start_position, selected_goal, current_path, pathfinding_mode
    start_position = None
    selected_goal = None
    current_path = []
    pathfinding_mode = False
...
def on_start_sim_click(button):
    """Callback for start simulation button click"""
    global start_sim_button, stop_sim_button
    if not customer_simulation['running']:
        start_customer_simulation()
        start_sim_button.enabled = False
        stop_sim_button.enabled = True

def on_stop_sim_click(button):
    """Callback for stop simulation button click"""
    global start_sim_button, stop_sim_button
    if customer_simulation['running']:
...
def on_toggle_heatmap_click(button):
    """Callback for toggle heatmap button click"""
    global toggle_heatmap_button
...
def on_reset_heatmap_click(button):
    """Callback for reset heatmap button click"""
    reset_heatmap()
...
def main():
    ...
    train_button = Button(CONTROL_PANEL_X + 10, CONTROL_PANEL_Y + 140, 150, 40, 
                         ...
                         on_click=on_train_button_click)
    ...
    set_start_button = Button(CONTROL_PANEL_X + 10, CONTROL_PANEL_Y + 560, 180, 35, 
                             "Set Start", BLUE, enabled=False,
                             on_click=on_set_start_click)  # Start disabled, wider button
    
    clear_path_button = Button(CONTROL_PANEL_X + 200, CONTROL_PANEL_Y + 560, 180, 35, 
                              "Clear Path", RED, enabled=False,
                              on_click=on_clear_path_click)  # Start disabled, wider button
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:  # Left click
                    ...
                    # Check goal button clicks (these still need manual handling)
                    goal_clicked = False
                    ...
                    # Handle grid cell clicks for pathfinding
                    if not goal_clicked:
        ...
        train_button.handle_events(mouse_pos, mouse_pressed)
        set_start_button.handle_events(mouse_pos, mouse_pressed)
        clear_path_button.handle_events(mouse_pos, mouse_pressed)
        start_sim_button.handle_events(mouse_pos, mouse_pressed)
        stop_sim_button.handle_events(mouse_pos, mouse_pressed)
        toggle_heatmap_button.handle_events(mouse_pos, mouse_pressed)
        reset_heatmap_button.handle_events(mouse_pos, mouse_pressed)
